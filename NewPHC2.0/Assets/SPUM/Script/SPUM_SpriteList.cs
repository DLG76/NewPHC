...
public class SPUM_SpriteList : MonoBehaviour
{
    ...
    public SpriteRenderer GetSpriteWithName(string name)
    {
        foreach (var s in _itemList)
            if (s.name.Equals(name))
                return s;
        foreach (var s in _eyeList)
            if (s.name.Equals(name))
                return s;
        foreach (var s in _hairList)
            if (s.name.Equals(name))
                return s;
        foreach (var s in _bodyList)
            if (s.name.Equals(name))
                return s;
        foreach (var s in _clothList)
            if (s.name.Equals(name))
                return s;
        foreach (var s in _armorList)
            if (s.name.Equals(name))
                return s;
        foreach (var s in _pantList)
            if (s.name.Equals(name))
                return s;
        foreach (var s in _weaponList)
            if (s.name.Equals(name))
                return s;
        foreach (var s in _backList)
            if (s.name.Equals(name))
                return s;
    ...
    public void SetSprite(string name, SpriteRenderer spriteRenderer2)
    {
        var spriteRenderer1 = GetSpriteWithName(name);

        if (spriteRenderer1 == null) return;

        SetSprite(spriteRenderer1, spriteRenderer2);
    }

    public void SetSprite(SpriteRenderer spriteRenderer1, SpriteRenderer spriteRenderer2)
    {
